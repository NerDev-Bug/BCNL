...
function OrdersPending() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        console.error("OrdersPending listener error:", err);
  ...
  const confirmAcceptOrder = async () => {
    ...
    try {
      ...
      await updateDoc(orderRef, { paymentStatus: "preparing" });
      ...
      toast.success("Order marked as preparing and customer notified!");
    } catch (err) {
  ...
  const confirmDeleteOrder = async () => {
    ...
    try {
      ...
      if (orderSnap.exists()) {
        const orderData = { id: orderSnap.id, ...orderSnap.data() };
        try {
          await createOrderCancelledNotification(orderData);
        } catch (notifErr) {
